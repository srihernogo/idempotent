...
import io.github.srihernogo.idempotent.core.aspect.IdempotentAspect;
import io.github.srihernogo.idempotent.core.persistence.IdempotentStore;
import io.github.srihernogo.idempotent.core.persistence.InMemoryIdempotentStore;
...
class IdempotentCoreAutoConfiguration {
    ...
    /**
     * Registers the {@link IdempotentAspect} only when Spring AOP is on the
     * classpath. Without
     * {@code spring-aop} (typically pulled in via {@code spring-boot-starter-aop})
     * the aspect
     * cannot be woven into method invocations, so registering it would create a
     * silent
     * footgun. Users relying on the programmatic {@link IdempotentService} are
     * unaffected.
     */
    @Bean
