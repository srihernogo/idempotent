package io.github.srihernogo.idempotent.core.aspect;

import io.github.srihernogo.idempotent.core.IdempotentProperties;
...
import io.github.srihernogo.idempotent.core.service.IdempotentService;
...
/**
 * AOP aspect that applies idempotency to methods annotated with
 * {@link Idempotent}. Resolves the
 * key (header takes precedence over the SpEL-evaluated annotation key), the
 * TTL, and the process
 * name from the join point, then delegates the state machine to
 * {@link IdempotentService}.
 */
@Aspect
public class IdempotentAspect {
    ...
    @Around("@annotation(io.github.srihernogo.idempotent.core.annotation.Idempotent)")
    ...
    /**
     * Parses {@code value} accepting both ISO-8601 ({@code PT1M}) and Spring's
     * short form
