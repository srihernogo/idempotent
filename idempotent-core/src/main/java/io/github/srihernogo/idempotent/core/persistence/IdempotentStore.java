 ...
 * <h2>Lifecycle</h2>
 * <ol>
 * <li>{@link #store store} performs a strict insert and is the only operation
 * that creates a new
 * entry. Conflicts raise {@link IdempotentKeyConflictException}.</li>
 * <li>{@link #update update} mutates an existing entry. It is a <strong>no-op
 * when the key is
 * missing</strong>; it never resurrects a deleted or expired entry.</li>
 * <li>{@link #loadValue loadValue} reads the persisted entry as-is.
 * {@link #getValue getValue} is
 * a default that wraps it with expiry enforcement, so implementations only
 * provide the raw
 * read.</li>
 * <li>{@link #remove remove} deletes the entry. It is idempotent and tolerates
 * missing keys.</li>
 * </ol>
 *
 * <h2>Implementation notes</h2>
 * <ul>
 * <li>Implementations provide {@link #loadValue} (raw persistence) and must
 * <strong>not</strong>
 * filter on {@code expiresAt} — expiry is enforced centrally by the default
 * {@link #getValue}.</li>
 * <li>The lazy delete inside the default {@code getValue} is opportunistic;
 * failures are logged
 * and swallowed.</li>
 * <li>Expiry uses wall-clock time via {@code expiresAt}; keep nodes
 * clock-synchronized (NTP) and
...
public interface IdempotentStore {
    ...
    /**
     * Reads the persisted entry for {@code key} without evaluating expiry.
     * Implemented by each
    ...
    /**
     * Reads the entry for {@code key}, returning {@code null} when missing or
     * expired. Expired
     * entries are removed as a best-effort cleanup so subsequent strict inserts can
     * reuse the key.
     *
     * <p>
     * This is a default that wraps {@link #loadValue}; implementations normally do
     * not override
     * it.
     *
     * @param key        the idempotent key
     ...
     * IdempotentKeyConflictException} when the key already exists rather than
     * overwriting.
     *
     * @param key   the idempotent key
     * @param value the value to store
     * @throws IdempotentKeyConflictException if the key already exists
     * @throws IdempotentException            if the backend fails for any other
     *                                        reason
     */
    void store(IdempotentKey key, Value value);

    /**
     * Deletes the entry for {@code key}. Idempotent: tolerates a missing key
     * without throwing.
     */
    void remove(IdempotentKey key);

    /**
     * Mutates an existing entry. <strong>No-op when the key is missing.</strong>
     * Implementations
     ...
     * @param processName logical scope, typically
     *                    {@code __ControllerName.methodName()}
     */
     ...
     * @param response  cached response, or {@code null} when no response has been
     *                  recorded yet
     *                  (typically while {@link Status#IN_PROGRESS}) or when the
     *                  response is null
     */
