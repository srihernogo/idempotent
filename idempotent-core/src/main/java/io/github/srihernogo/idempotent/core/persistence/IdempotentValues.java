...
import static io.github.srihernogo.idempotent.core.persistence.IdempotentStore.Value;
...
public final class IdempotentValues {
    ...
    /**
     * Returns the value when valid, or {@code null} when missing or expired.
     * Expired entries are
     * removed via {@code removeExpired} as a best-effort cleanup so subsequent
     * strict inserts can
     * reuse the key. Cleanup failures are logged and swallowed; a {@code getValue}
     * read must not
