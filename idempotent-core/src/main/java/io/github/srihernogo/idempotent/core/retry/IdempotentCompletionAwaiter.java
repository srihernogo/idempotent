...
/**
 * Waits for an in-progress idempotent operation to complete using exponential
 * backoff.
 */
public class IdempotentCompletionAwaiter {
     ...
     * @param value         current in-progress value (must be non-null); the
     *                      awaiter polls the
     *                      store until the entry transitions to {@code COMPLETED}
     *                      or the retry
     *                      budget is exhausted
     * @return the latest value from the store, or {@code null} if the entry was
     *         removed (e.g.,
