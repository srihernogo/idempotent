package io.github.srihernogo.idempotent.core.service;
...
import io.github.srihernogo.idempotent.core.exception.IdempotentWaitExhaustedException;
import io.github.srihernogo.idempotent.core.persistence.IdempotentStore;
import io.github.srihernogo.idempotent.core.retry.IdempotentCompletionAwaiter;
import io.github.srihernogo.idempotent.core.retry.WaitStrategy;
...
import static io.github.srihernogo.idempotent.core.persistence.IdempotentStore.Status.COMPLETED;
import static io.github.srihernogo.idempotent.core.persistence.IdempotentStore.Status.IN_PROGRESS;
 ...
 * The untyped
 * {@link #execute(IdempotentStore.IdempotentKey, Supplier, Duration) execute}
 * overloads
 * pass {@code Object.class} to the store and rely on the codec's polymorphic
 * typing to reconstruct
 * the concrete response type. The typed overloads accept a {@link Class} hint
 * and let stores that
 * support typed deserialization (RDS, DynamoDB) round-trip without relying on
 * polymorphic
 * {@code @class} metadata. Prefer the typed overloads when the response type is
 * known.
 *
 * <h2>Exception propagation</h2>
 * Domain exceptions thrown by the operation propagate to the caller as-is (no
 * wrapping in
 * {@link IdempotentException}). Cleanup of the in-progress entry happens before
 * the throw.
 */
public class IdempotentService {
    ...
    /**
     * Canonical idempotent execution that accepts an operation able to throw any
     * {@link Throwable}.
     * Used by {@code IdempotentAspect} to forward
     * {@code ProceedingJoinPoint::proceed} without
    ...
    private void updateStoreWithResponse(
        ...
        if (response instanceof ResponseEntity<?> responseEntity
            ...
            // Non-2xx responses are treated as failures and not cached so the caller can
            // retry.
            idempotentStore.remove(idempotentKey);
            return;
        }
        // Cache the result — including null (void methods or intentional null returns)
        // so
